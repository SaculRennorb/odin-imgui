...
when ALLEGRO_HAS_DRAW_INDEXED_PRIM {
        ...
        if (size_of(ImDrawIdx) == 2)
        {
            ...
            for i32 i = 0; i < draw_list.IdxBuffer.Size; ++i
                bd.BufIndices[i] = cast(ast) ast) list listuffer.Data[i];
        ...
        for i32 i = 0; i < draw_list.IdxBuffer.Size; i++
        {
            src_v := &draw_list.VtxBuffer[draw_list.IdxBuffer[i]];
            dst_v := &vertices[i];
            DRAW_VERT_IMGUI_TO_ALLEGRO(dst_v, src_v);
        }
}
...
ImGui_ImplAllegro5_KeyCodeToImGuiKey := ImGui_Im(Im( key_code);
ImGui_ImplAllegro5_KeyCodeToImGuiKey := ImGui_Im(Im( key_code)
{
    switch (key_code)
    {
    ...
    case ALLEGRO_EVENT_KEY_CHAR:
        if (ev.keyboard.display == bd.Display)
            if (ev.keyboard.unichar != 0)
                io.AddInputCharacter(cast(ast) ast) asoard.unichar);
        return true;
