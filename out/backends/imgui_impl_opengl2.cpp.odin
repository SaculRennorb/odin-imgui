...
ImGui_ImplOpenGL2_RenderDrawData :: proc(draw_data : ^ImDrawData)
{
    ...
    // Backup GL state
    last_texture : GLint
    last_polygon_mode : [2]GLint glGetIntegerv(GL_POLYGON_MODE, last_polygon_mode);
    ...
    for i32 n = 0; n < draw_data.CmdListsCount; n++
    {
        ...
        glVertexPointer(2, GL_FLOAT, size_of(ImDrawVert), (const GLrawptr)((const u8*)vtx_buffer + offsetof(ImDrawVert, pos)));
        glTexCoordPointer(2, GL_FLOAT, size_of(ImDrawVert), (const GLrawptr)((const u8*)vtx_buffer + offsetof(ImDrawVert, uv)));
        glColorPointer(4, GL_UNSIGNED_BYTE, size_of(ImDrawVert), (const GLrawptr)((const u8*)vtx_buffer + offsetof(ImDrawVert, col)));
        ...
        for i32 cmd_i = 0; cmd_i < draw_list.CmdBuffer.Size; cmd_i++
        {
            ...
            else
            {
                ...
                glScissor(cast(ast) ast) min) mini32)(cast(32)) cast(32)) cast(32))x.y), (i32)(clip_max.x - clip_min.x), (i32)(clip_max.y - clip_min.y));
