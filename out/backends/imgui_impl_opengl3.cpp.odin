...
when !defined(IMGUI_IMPL_OPENGL_ES2) && !defined(IMGUI_IMPL_OPENGL_ES3) && !defined(IMGUI_IMPL_OPENGL_LOADER_CUSTOM) {
    ...
    gl_version_str := (const u8*)glGetString(GL_VERSION);
when defined(IMGUI_IMPL_OPENGL_ES2) {
    // GLES 2
    bd.GlVersion = 200;
...
when _WIN32 {
    if (const u8* vendor = (const u8*)glGetString(GL_VENDOR))
...
when IMGUI_IMPL_OPENGL_DEBUG {
    printf("GlVersion = %d, \"%s\"\nGlProfileIsCompat = %d\nGlProfileMask = 0x%X\nGlProfileIsES2/IsEs3 = %d/%d\nGL_VENDOR = '%s'\nGL_RENDERER = '%s'\n", bd.GlVersion, gl_version_str, bd.GlProfileIsCompat, bd.GlProfileMask, bd.GlProfileIsES2, bd.GlProfileIsES3, (const u8*)glGetString(GL_VENDOR), (const u8*)glGetString(GL_RENDERER)); // [DEBUG]
}
...
when defined(IMGUI_IMPL_OPENGL_ES2) {
    ...
    assert(cast(ast) ast) nst) n_version) + 2 < len(bd.GlslVersionString));
...
when IMGUI_IMPL_OPENGL_HAS_EXTENSIONS {
    ...
    {
        extension := (const u8*)glGetStringi(GL_EXTENSIONS, i);
...
when IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY {
    ...
    GL_CALL(glVertexAttribPointer(bd.AttribLocationVtxPos,   2, GL_FLOAT,         GL_FALSE, size_of(ImDrawVert), (GLrawptr)offsetof(ImDrawVert, pos)));
    GL_CALL(glVertexAttribPointer(bd.AttribLocationVtxUV,    2, GL_FLOAT,         GL_FALSE, size_of(ImDrawVert), (GLrawptr)offsetof(ImDrawVert, uv)));
    GL_CALL(glVertexAttribPointer(bd.AttribLocationVtxColor, 4, GL_UNSIGNED_BYTE, GL_TRUE, size_of(ImDrawVert), (GLrawptr)offsetof(ImDrawVert, col)));
}
...
when IMGUI_IMPL_OPENGL_MAY_HAVE_POLYGON_MODE {
    last_polygon_mode : [2]GLint if (bd.HasPolygonMode) { glGetIntegerv(GL_POLYGON_MODE, last_polygon_mode); }
}
    last_viewport : [4]GLint glGetIntegerv(GL_VIEWPORT, last_viewport);
    last_scissor_box : [4]GLint glGetIntegerv(GL_SCISSOR_BOX, last_scissor_box);
...
when IMGUI_IMPL_OPENGL_USE_VERTEX_ARRAY {
    ...
    for i32 n = 0; n < draw_data.CmdListsCount; n++
    {
        ...
        vtx_buffer_size := (GLsizeiptr)draw_list.VtxBuffer.Size * cast(ast) ast) oft) ofawVert);
        idx_buffer_size := (GLsizeiptr)draw_list.IdxBuffer.Size * cast(ast) ast) oft) ofawIdx);
        ...
        for i32 cmd_i = 0; cmd_i < draw_list.CmdBuffer.Size; cmd_i++
        {
            ...
            else
            {
                ...
                GL_CALL(glScissor(cast(ast) ast) min) mini32)(cast(32)) cast(32)) cast(32))x.y), (i32)(clip_max.x - clip_min.x), (i32)(clip_max.y - clip_min.y)));

                // Bind texture, Draw
                GL_CALL(glBindTexture(GL_TEXTURE_2D, (GLuint)(intptr_t)pcmd.GetTexID()));
when IMGUI_IMPL_OPENGL_MAY_HAVE_VTX_OFFSET {
                if (bd.GlVersion >= 320)
                    GL_CALL(glDrawElementsBaseVertex(GL_TRIANGLES, (GLsizei)pcmd.ElemCount, size_of(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, (rawptr)(intptr_t)(pcmd.IdxOffset * size_of(ImDrawIdx)), (GLint)pcmd.VtxOffset));
                else
}
                GL_CALL(glDrawElements(GL_TRIANGLES, (GLsizei)pcmd.ElemCount, size_of(ImDrawIdx) == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, (rawptr)(intptr_t)(pcmd.IdxOffset * size_of(ImDrawIdx))));
            }
        }
    }
