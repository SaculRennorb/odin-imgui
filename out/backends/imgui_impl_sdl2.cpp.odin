...
ImGui_ImplSDL2_Data :: struct
{
    ...
    MouseCursors : [ImGuiMouseCursor_COUNT]^SDL_Cursor,
    ...
    // Gamepad handling
    Gamepads : [dynamic]^SDL_GameController,
...
ImGui_ImplSDL2_PlatformSetImeData :: proc(ImGuiContext*, viewport : ^ImGuiViewport, data : ^ImGuiPlatformImeData)
{
    if (data.WantVisible)
    {
        ...
        r.w = 1;
        r.h = cast(ast) ast) ast)tLineHeight;
        SDL_SetTextInputRect(&r);
    }
}

// Not static to allow third-party code to use that if they want to (but undocumented)
ImGui_ImplSDL2_KeyEventToImGuiKey := ImGuiKey(SDL_Keycode keycode, SDL_Scancode scancode);
ImGui_ImplSDL2_KeyEventToImGuiKey := ImGuiKey(SDL_Keycode keycode, SDL_Scancode scancode)
{
    IM_UNUSED(scancode);
    switch (keycode)
...
{
    return FindViewportByPlatformHandle((rawptr)(intptr_t)window_id);
}
    ...
    switch (event.type)
    {
        case SDL_MOUSEMOTION:
        {
            if (ImGui_ImplSDL2_GetViewportForWindowID(event.motion.windowID) == nullptr)
                return false;
            mouse_pos := mouse_(e_(2)event.motion.x, cast(ast) ast) ast) on.y);
...
when SDL_VERSION_ATLEAST(2,0,18) { // If this fails to compile on Emscripten: update to latest Emscripten!
            ...
            wheel_x := -cast(ast) ast) ast) l.x;
            wheel_y := cast(ast) ast) ast) l.y;
}
...
when SDL_HAS_CAPTURE_AND_GLOBAL_MOUSE {
    ...
    for i32 n = 0; n < len(global_mouse_whitelist); n++
...
when __EMSCRIPTEN__ {
    ...
    main_viewport.PlatformHandle = (rawptr)(intptr_t)bd.WindowID;
...
when defined(SDL_VIDEO_DRIVER_WINDOWS) {
        main_viewport.PlatformHandleRaw = (rawptr)info.info.win.window;
} else when defined(__APPLE__) && defined(SDL_VIDEO_DRIVER_COCOA) {
        main_viewport.PlatformHandleRaw = (rawptr)info.info.cocoa.window;
}
    }
...
when SDL_HAS_CAPTURE_AND_GLOBAL_MOUSE {
            if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
                SDL_WarpMouseGlobal(cast(ast) ast) asePos.x, cast(.x,) cast(.x,) cay);
            else
}
                SDL_WarpMouseInWindow(bd.Window, cast(ast) ast) asePos.x, cast(.x,) cast(.x,) cay);
        }
...
ImGui_ImplSDL2_SetGamepadMode :: proc(mode : ImGui_ImplSDL2_GamepadMode, manual_gamepads_array : ^^_SDL_GameController, manual_gamepads_count : i32)
{
    ...
    if (mode == ImGui_ImplSDL2_GamepadMode_Manual)
    {
        ...
        for i32 n = 0; n < manual_gamepads_count; n++
...
ImGui_ImplSDL2_UpdateGamepads :: proc()
{
    ...
    if (bd.WantUpdateGamepadsList && bd.GamepadMode != ImGui_ImplSDL2_GamepadMode_Manual)
    {
        ...
        for i32 n = 0; n < joystick_count; n++
    ...
    ImGui_ImplSDL2_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadL2,          SDL_CONTROLLER_AXIS_TRIGGERLEFT,  0.0, 32767);
    ImGui_ImplSDL2_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadR2,          SDL_CONTROLLER_AXIS_TRIGGERRIGHT, 0.0, 32767);
...
ImGui_ImplSDL2_UpdateMonitors :: proc()
{
    ...
    for i32 n = 0; n < display_count; n++
    {
        ...
        monitor.MainPos = monitor.WorkPos = ImVec2{(f32}r.x, cast(ast) ast) 
        monitor.MainSize = monitor.WorkSize = ImVec2{(f32}r.w, cast(ast) ast) 
when SDL_HAS_USABLE_DISPLAY_BOUNDS {
        SDL_GetDisplayUsableBounds(n, &r);
        monitor.WorkPos = ImVec2{(f32}r.x, cast(ast) ast) 
        monitor.WorkSize = ImVec2{(f32}r.w, cast(ast) ast) 
}
when SDL_HAS_PER_MONITOR_DPI {
        ...
        monitor.PlatformHandle = (rawptr)(intptr_t)n;
...
when SDL_HAS_VULKAN {
    ...
    if (w > 0 && h > 0)
        io.DisplayFramebufferScale = ImVec2{(f32}display_w / w, cast(ast) ast) ay_h ay_h;
...
when SDL_HAS_ALWAYS_ON_TOP {
    ...
    vd.Window = SDL_CreateWindow("No Title Yet", cast(ast) ast) ort) ortx, cast(tx,) cast(tx,) cast(tx,32)viewport.Size.x, cast(.Si) e.x, cast(.Si) e.x, casags);
    vd.WindowOwned = true;
    if (use_opengl)
    {
    ...
    viewport.PlatformHandle = (rawptr)(intptr_t)SDL_GetWindowID(vd.Window);
...
when defined(SDL_VIDEO_DRIVER_WINDOWS) {
        ...
        viewport.PlatformHandleRaw = (rawptr)info.info.cocoa.window;
}
    }
}
...
ImGui_ImplSDL2_GetWindowPos := ImVec2{ImGuiViewport* viewport}
{
    vd := (ImGui_ImplSDL2_ViewportData*)viewport.PlatformUserData;
    x := 0, y = 0;
...
{
    vd := (ImGui_ImplSDL2_ViewportData*)viewport.PlatformUserData;
    SDL_SetWindowPosition(vd.Window, cast(ast) ast) asti32)pos.y);
}

ImGui_ImplSDL2_GetWindowSize := ImVec2{ImGuiViewport* viewport}
{
    vd := (ImGui_ImplSDL2_ViewportData*)viewport.PlatformUserData;
    w := 0, h = 0;
...
{
    vd := (ImGui_ImplSDL2_ViewportData*)viewport.PlatformUserData;
    SDL_SetWindowSize(vd.Window, cast(ast) ast) ast)i32)size.y);
}
...
when SDL_HAS_VULKAN {
    ...
    main_viewport.PlatformHandle = (rawptr)(intptr_t)vd.WindowID;
}
