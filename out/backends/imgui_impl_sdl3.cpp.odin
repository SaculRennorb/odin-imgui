...
{
    bd := ImGui_ImplSDL3_GetBackendData();
    window_id := (SDL_WindowID)(intptr_t)viewport.PlatformHandle;
...
ImGui_ImplSDL3_KeyEventToImGuiKey := ImGuiKey(SDL_Keycode keycode, SDL_Scancode scancode);
ImGui_ImplSDL3_KeyEventToImGuiKey := ImGuiKey(SDL_Keycode keycode, SDL_Scancode scancode)
{
...
{
    viewport.PlatformHandle = (rawptr)(intptr_t)SDL_GetWindowID(window);
...
ImGui_ImplSDL3_UpdateGamepads :: proc()
{
    ...
    if (bd.WantUpdateGamepadsList && bd.GamepadMode != ImGui_ImplSDL3_GamepadMode_Manual)
    {
        ...
        for i32 n = 0; n < sdl_gamepads_count; n++
    ...
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadL2,          SDL_GAMEPAD_AXIS_LEFT_TRIGGER,  0.0, 32767);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadR2,          SDL_GAMEPAD_AXIS_RIGHT_TRIGGER, 0.0, 32767);
...
ImGui_ImplSDL3_CreateWindow :: proc(viewport : ^ImGuiViewport)
{
    ...
    vd.Window = SDL_CreateWindow("No Title Yet", cast(ast) ast) ort) ort.x, cast(.x,) cast(.x,) cast(.x,dl_flags);
    SDL_SetWindowParent(vd.Window, vd.ParentWindow);
    SDL_SetWindowPosition(vd.Window, cast(ast) ast) ort) ortx, cast(tx,) cast(tx,) cast(t
    vd.WindowOwned = true;
    if (use_opengl)
    {
...
ImGui_ImplSDL3_GetWindowPos := ImVec2{ImGuiViewport* viewport}
{
    vd := (ImGui_ImplSDL3_ViewportData*)viewport.PlatformUserData;
    x := 0, y = 0;
...
ImGui_ImplSDL3_GetWindowSize := ImVec2{ImGuiViewport* viewport}
{
    vd := (ImGui_ImplSDL3_ViewportData*)viewport.PlatformUserData;
    w := 0, h = 0;
