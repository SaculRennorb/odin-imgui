...
ImGui_ImplWin32_InitEx :: proc(hwnd : rawptr, platform_has_own_dc : bool) -> bool
{
    ...
    main_viewport.PlatformHandle = main_viewport.PlatformHandleRaw = (rawptr)bd.hWnd;
...
when !(IMGUI_IMPL_WIN32_DISABLE_GAMEPAD) {
    ...
    for i32 n = 0; n < len(xinput_dll_names); n++
...
ImGui_ImplWin32_UpdateMouseData :: proc(io : ^ImGuiIO, platform_io : ^ImGuiPlatformIO)
{
    ...
    if (is_app_focused)
    {
        ...
        {
            pos := { cast(ast) ast) asePos.x, cast(.x,) cast(.x,) cay };
        ...
        if (!io.WantSetMousePos && bd.MouseTrackedArea == 0 && has_mouse_screen_pos)
        {
            ...
            io.AddMousePosEvent(cast(ast) ast) _pos _posf32)mouse_pos.y);
        }
    }
...
BOOL CALLBACK ImGui_ImplWin32_UpdateMonitors_EnumFunc(HMONITOR monitor, HDC, LPRECT, LPARAM)
{
    ...
    ImGuiPlatformIO& io = GetPlatformIO();
...
ImGui_ImplWin32_KeyEventToImGuiKey := ImGuiKey(WPARAM wParam, LPARAM lParam);
ImGui_ImplWin32_KeyEventToImGuiKey := ImGuiKey(WPARAM wParam, LPARAM lParam)
{
...
ImGui_ImplWin32_GetMouseSourceFromMessageExtraInfo := ImGuiMouseSource()
{
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerEx(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, ImGuiIO& io)
{
    ...
    case WM_NCMOUSEMOVE:
    {
        ...
        want_absolute_pos := (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) != 0;
    ...
    case WM_MOUSEWHEEL:
        io.AddMouseWheelEvent(0.0, cast(ast) ast) HEEL_DELTA_WPARAMPARAMam) / cast() /) cast() /) Aas
        return 0;
    case WM_MOUSEHWHEEL:
        io.AddMouseWheelEvent(-cast(ast) ast) HEEL_DELTA_WPARAMPARAMam) / cast() /) cast() /) Aast() /
        return 0;
    case WM_KEYDOWN:
    case WM_KEYUP:
    case WM_SYSKEYDOWN:
    case WM_SYSKEYUP:
    {
        ...
        if (wParam < 256)
        {
            ...
            vk := cast(ast) ast) ms
            scancode := cast(ast) ast) Est) ERD(lParam));
...
ImGui_ImplWin32_UpdateWindow :: proc(viewport : ^ImGuiViewport)
{
    ...
    if (vd.DwStyle != new_style || vd.DwExStyle != new_ex_style)
    {
        ...
        insert_after := top_most_changed ? ((viewport.Flags & ImGuiViewportFlags_TopMost) ? HWND_TOPMOST : HWND_NOTOPMOST) : 0;
...
ImGui_ImplWin32_GetWindowPos := ImVec2{ImGuiViewport* viewport}
{
    vd := (ImGui_ImplWin32_ViewportData*)viewport.PlatformUserData;
    assert(vd.Hwnd != 0);
    pos := { 0, 0 };
    ::ClientToScreen(vd.Hwnd, &pos);
    return ImVec2{(f32}pos.x, cast(ast) ast) as
}
...
ImGui_ImplWin32_GetWindowSize := ImVec2{ImGuiViewport* viewport}
{
    vd := (ImGui_ImplWin32_ViewportData*)viewport.PlatformUserData;
    assert(vd.Hwnd != 0);
    rect : RECT
...
LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    ImGuiIO& io = GetIOEx(ctx);
    ImGuiPlatformIO& platform_io = GetPlatformIOEx(ctx);
    result := 0;
