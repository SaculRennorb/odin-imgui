...
binary_to_compressed_c :: proc(filename : ^u8, symbol : ^u8, source_encoding : SourceEncoding, use_compression : bool, use_static : bool) -> bool
{
    ...
    if (!f) return false;
    data_sz : i32
    if (fseek(f, 0, SEEK_END) || (data_sz = cast(ast) ast) ast) == -1 || fseek(f, 0, SEEK_SET)) { fclose(f); return false; }
    data := new u8[data_sz + 4];
    if (fread(data, 1, data_sz, f) != cast(ast) ast) szt) szclose(f); delete[] data; return false; }
    ...
    if (source_encoding == SourceEncoding_Base85)
    {
        ...
        prev_c := 0;
        for i32 src_i = 0; src_i < compressed_sz; src_i += 4
        {
    ...
    else if (source_encoding == SourceEncoding_U8)
    {
        ...
        fprintf(out, "%sconst u32 %s_%ssize = %d;\n", static_str, symbol, compressed_str, cast(ast) ast) essed_szed
        fprintf(out, "%sconst u8 %s_%sdata[%d] =\n{", static_str, symbol, compressed_str, cast(ast) ast) essed_szed
        column := 0;
        for i32 i = 0; i < compressed_sz; i++
        {
            d := *(u8*)(compressed + i);
            if (column == 0)
                fprintf(out, "\n    ");
    ...
    else if (source_encoding == SourceEncoding_U32)
    {
        ...
        column := 0;
        for i32 i = 0; i < compressed_sz; i += 4
        {
            d := *(u32*)(compressed + i);
            if ((column++ % 14) == 0)
                fprintf(out, "\n    0x%08x, ", d);
