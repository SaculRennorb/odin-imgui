...
// DirectX data
ImGui_ImplDX10_Data :: struct
{
    pd3dDevice : ^ID3D10Device,
    pFactory : ^IDXGIFactory,
    pVB : ^ID3D10Buffer,
    pIB : ^ID3D10Buffer,
    pVertexShader : ^ID3D10VertexShader,
    pInputLayout : ^ID3D10InputLayout,
    pVertexConstantBuffer : ^ID3D10Buffer,
    pPixelShader : ^ID3D10PixelShader,
    pFontSampler : ^ID3D10SamplerState,
    pFontTextureView : ^ID3D10ShaderResourceView,
    ...
    ImGui_ImplDX10_Data()       { memset((rawptr)this, 0, size_of(*this)); VertexBufferSize = 5000; IndexBufferSize = 10000; }
};

VERTEX_CONSTANT_BUFFER_DX10 :: struct
{
    f32   mvp[4][4];
};
...
ImGui_ImplDX10_GetBackendData :: proc() -> ^ImGui_ImplDX10_Data
{
    return GetCurrentContext() ? (ImGui_ImplDX10_Data*)GetIO().BackendRendererUserData : nullptr;
}
...
// Functions
ImGui_ImplDX10_SetupRenderState :: proc(draw_data : ^ImDrawData, device : ^ID3D10Device)
{
    bd := ImGui_ImplDX10_GetBackendData();

    // Setup viewport
    vp : D3D10_VIEWPORT
    memset(&vp, 0, size_of(D3D10_VIEWPORT));
    ...
    mapped_resource : rawptr
    if (bd.pVertexConstantBuffer.Map(D3D10_MAP_WRITE_DISCARD, 0, &mapped_resource) == S_OK)
    {
        constant_buffer := (VERTEX_CONSTANT_BUFFER_DX10*)mapped_resource;
    ...
    device.IASetIndexBuffer(bd.pIB, size_of(ImDrawIdx) == 2 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT, 0);
...
// Render function
ImGui_ImplDX10_RenderDrawData :: proc(draw_data : ^ImDrawData)
{
    ...
    if (!bd.pVB || bd.VertexBufferSize < draw_data.TotalVtxCount)
    {
        ...
        memset(&desc, 0, size_of(D3D10_BUFFER_DESC));
        desc.Usage = D3D10_USAGE_DYNAMIC;
        desc.ByteWidth = bd.VertexBufferSize * size_of(ImDrawVert);
        desc.BindFlags = D3D10_BIND_VERTEX_BUFFER;
        desc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
        desc.MiscFlags = 0;
        if (device.CreateBuffer(&desc, nullptr, &bd.pVB) < 0)   do return
    }
    ...
    if (!bd.pIB || bd.IndexBufferSize < draw_data.TotalIdxCount)
    {
        ...
        desc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
        if (device.CreateBuffer(&desc, nullptr, &bd.pIB) < 0)   do return
    }

    // Copy and convert all vertices into a single contiguous buffer
    vtx_dst := nullptr;
    idx_dst := nullptr;
    bd.pVB.Map(D3D10_MAP_WRITE_DISCARD, 0, (rawptr*)&vtx_dst);
    bd.pIB.Map(D3D10_MAP_WRITE_DISCARD, 0, (rawptr*)&idx_dst);
    ...
    BACKUP_DX10_STATE :: struct
    {
        ScissorRectsCount, ViewportsCount : UINT,
        D3D10_RECT                  ScissorRects[D3D10_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE];
        D3D10_VIEWPORT              Viewports[D3D10_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE];
        RS : ^ID3D10RasterizerState,
        BlendState : ^ID3D10BlendState,
        FLOAT                       BlendFactor[4];
        SampleMask : UINT,
        StencilRef : UINT,
        DepthStencilState : ^ID3D10DepthStencilState,
        PSShaderResource : ^ID3D10ShaderResourceView,
        PSSampler : ^ID3D10SamplerState,
        PS : ^ID3D10PixelShader,
        VS : ^ID3D10VertexShader,
        GS : ^ID3D10GeometryShader,
        PrimitiveTopology : D3D10_PRIMITIVE_TOPOLOGY,
    ...
    render_state : ImGui_ImplDX10_RenderState
    ...
    for int n = 0; n < draw_data.CmdListsCount; n += 1
    {
        ...
        for int cmd_i = 0; cmd_i < len(draw_list.CmdBuffer); cmd_i += 1
        {
            ...
            else
            {
                ...
                // Bind texture, Draw
                texture_srv := (ID3D10ShaderResourceView*)pcmd.GetTexID();
...
ImGui_ImplDX10_CreateFontsTexture :: proc()
{
    // Build texture atlas
    bd := ImGui_ImplDX10_GetBackendData();
    io := GetIO();
    ...
    // Upload texture to graphics system
    {
        ...
        pTexture := nullptr;
        subResource : D3D10_SUBRESOURCE_DATA
        ...
        // Create texture view
        srv_desc : D3D10_SHADER_RESOURCE_VIEW_DESC
        ZeroMemory(&srv_desc, size_of(srv_desc));
...
ImGui_ImplDX10_DestroyFontsTexture :: proc()
{
    bd := ImGui_ImplDX10_GetBackendData();
    if (bd.pFontTextureView)
    {
        ...
        GetIO().Fonts.SetTexID(0); // We copied bd.pFontTextureView to io.Fonts.TexID so let's clear that as well.
    }
}

ImGui_ImplDX10_CreateDeviceObjects :: proc() -> bool
{
    bd := ImGui_ImplDX10_GetBackendData();
    if (!bd.pd3dDevice)   do return false
    if (bd.pFontSampler)
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), nullptr, nullptr, nullptr, "main", "vs_4_0", 0, 0, &vertexShaderBlob, nullptr)))   do return false // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob.GetBufferPointer(). Make sure to Release() the blob!
        ...
        {
            { "POSITION", 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)offset_of(ImDrawVert, pos), D3D10_INPUT_PER_VERTEX_DATA, 0 },
            { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)offset_of(ImDrawVert, uv),  D3D10_INPUT_PER_VERTEX_DATA, 0 },
            { "COLOR",    0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, (UINT)offset_of(ImDrawVert, col), D3D10_INPUT_PER_VERTEX_DATA, 0 },
        };
        ...
        {
            desc : D3D10_BUFFER_DESC
            desc.ByteWidth = size_of(VERTEX_CONSTANT_BUFFER_DX10);
            desc.Usage = D3D10_USAGE_DYNAMIC;
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), nullptr, nullptr, nullptr, "main", "ps_4_0", 0, 0, &pixelShaderBlob, nullptr)))   do return false // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob.GetBufferPointer(). Make sure to Release() the blob!
...
ImGui_ImplDX10_InvalidateDeviceObjects :: proc()
{
    bd := ImGui_ImplDX10_GetBackendData();
    if (!bd.pd3dDevice)   do return
...
ImGui_ImplDX10_Init :: proc(device : ^ID3D10Device) -> bool
{
    io := GetIO();
    IMGUI_CHECKVERSION();
...
ImGui_ImplDX10_NewFrame :: proc()
{
    bd := ImGui_ImplDX10_GetBackendData();
    assert(bd != nullptr, "Context or backend not initialized! Did you call ImGui_ImplDX10_Init()?");
...
ImGui_ImplDX10_ViewportData :: struct
{
    SwapChain : ^IDXGISwapChain,
    RTView : ^ID3D10RenderTargetView,

    ImGui_ImplDX10_ViewportData()   { SwapChain = nullptr; RTView = nullptr; }
    ~ImGui_ImplDX10_ViewportData()  { assert(SwapChain == nullptr && RTView == nullptr); }
};

ImGui_ImplDX10_CreateWindow :: proc(viewport : ^ImGuiViewport)
{
    bd := ImGui_ImplDX10_GetBackendData();
...
ImGui_ImplDX10_DestroyWindow :: proc(viewport : ^ImGuiViewport)
{
    ...
    if (ImGui_ImplDX10_ViewportData* vd = (ImGui_ImplDX10_ViewportData*)viewport.RendererUserData)
    {
        ...
        if (vd.RTView)   do vd.RTView.Release()
        vd.RTView = nullptr;
        IM_DELETE(vd);
    }
...
ImGui_ImplDX10_SetWindowSize :: proc(viewport : ^ImGuiViewport, size : ImVec2)
{
    bd := ImGui_ImplDX10_GetBackendData();
    vd := (ImGui_ImplDX10_ViewportData*)viewport.RendererUserData;
    if (vd.RTView)
    {
        vd.RTView.Release();
        vd.RTView = nullptr;
...
ImGui_ImplDX10_RenderViewport :: proc(viewport : ^ImGuiViewport, rawptr)
{
    bd := ImGui_ImplDX10_GetBackendData();
    vd := (ImGui_ImplDX10_ViewportData*)viewport.RendererUserData;
    ...
    if (!(.NoRendererClear in viewport.Flags)) {
        bd.pd3dDevice.ClearRenderTargetView(vd.RTView, cast(^f32) &clear_color);
    }

    ImGui_ImplDX10_RenderDrawData(viewport.DrawData);
}

ImGui_ImplDX10_SwapBuffers :: proc(viewport : ^ImGuiViewport, rawptr)
{
    vd := (ImGui_ImplDX10_ViewportData*)viewport.RendererUserData;
    vd.SwapChain.Present(0, 0); // Present without vsync
}

ImGui_ImplDX10_InitMultiViewportSupport :: proc()
{
    platform_io := &GetPlatformIO();
...
ImGui_ImplDX10_ShutdownMultiViewportSupport :: proc()
{
