...
ImGui_ImplDX11_RenderDrawData :: proc(draw_data : ^ImDrawData)
{
    ...
    if (!bd.pVB || bd.VertexBufferSize < draw_data.TotalVtxCount)
    {
        ...
        desc.MiscFlags = 0;
        if (bd.pd3dDevice.CreateBuffer(&desc, nullptr, &bd.pVB) < 0)   do return
    }
    if (!bd.pIB || bd.IndexBufferSize < draw_data.TotalIdxCount)
    {
        ...
        desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
        if (bd.pd3dDevice.CreateBuffer(&desc, nullptr, &bd.pIB) < 0)   do return
    }

    // Upload vertex/index data into a single contiguous GPU buffer
    vtx_resource, idx_resource : D3D11_MAPPED_SUBRESOURCE
    if (device.Map(bd.pVB, 0, D3D11_MAP_WRITE_DISCARD, 0, &vtx_resource) != S_OK)   do return
    if (device.Map(bd.pIB, 0, D3D11_MAP_WRITE_DISCARD, 0, &idx_resource) != S_OK)   do return
...
ImGui_ImplDX11_CreateDeviceObjects :: proc() -> bool
{
    ...
    // Create the vertex shader
    {
        ...
        {
            { "POSITION", 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)offset_of(ImDrawVert, pos), D3D11_INPUT_PER_VERTEX_DATA, 0 },
            { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)offset_of(ImDrawVert, uv),  D3D11_INPUT_PER_VERTEX_DATA, 0 },
            { "COLOR",    0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, (UINT)offset_of(ImDrawVert, col), D3D11_INPUT_PER_VERTEX_DATA, 0 },
        };
...
ImGui_ImplDX11_RenderWindow :: proc(viewport : ^ImGuiViewport, rawptr)
{
    ...
    if (!(.NoRendererClear in viewport.Flags)) {
        bd.pd3dDeviceContext.ClearRenderTargetView(vd.RTView, cast(^f32) &clear_color);
    }
