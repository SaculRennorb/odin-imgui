...
//  [X] Platform: Keyboard support. Since 1.87 we are using the io.AddKeyEvent() function. Pass ImGuiKey values to all key functions e.g. ImGui::IsKeyPressed(ImGuiKey.Space). [Legacy GLFW_KEY_* values are obsolete since 1.87 and not supported since 1.91.5]
...
ImGuiKey ImGui_ImplGlfw_KeyToImGuiKey(i32 keycode, i32 scancode);
ImGui_ImplGlfw_KeyToImGuiKey :: proc(keycode : i32, scancode : i32) -> ImGuiKey
{
    _ = scancode;
    switch (keycode)
    {
...
{
    io := GetIO();
    io.AddKeyEvent(ImGuiKey.Mod_Ctrl,  (glfwGetKey(window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS) || (glfwGetKey(window, GLFW_KEY_RIGHT_CONTROL) == GLFW_PRESS));
    io.AddKeyEvent(ImGuiKey.Mod_Shift, (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT)   == GLFW_PRESS) || (glfwGetKey(window, GLFW_KEY_RIGHT_SHIFT)   == GLFW_PRESS));
    io.AddKeyEvent(ImGuiKey.Mod_Alt,   (glfwGetKey(window, GLFW_KEY_LEFT_ALT)     == GLFW_PRESS) || (glfwGetKey(window, GLFW_KEY_RIGHT_ALT)     == GLFW_PRESS));
    io.AddKeyEvent(ImGuiKey.Mod_Super, (glfwGetKey(window, GLFW_KEY_LEFT_SUPER)   == GLFW_PRESS) || (glfwGetKey(window, GLFW_KEY_RIGHT_SUPER)   == GLFW_PRESS));
}
...
ImGui_ImplGlfw_MouseButtonCallback :: proc(window : ^GLFWwindow, button : i32, action : i32, mods : i32)
{
    ...
    if (bd.MouseIgnoreButtonUp && action == GLFW_RELEASE)   do return
...
when EMSCRIPTEN_USE_EMBEDDED_GLFW3 {
    ...
    io := GetIO();
    io.AddMouseWheelEvent(cast(f32) xoffset, cast(f32) yoffset);
}
...
when GLFW_HAS_GETKEYNAME && !defined(EMSCRIPTEN_USE_EMBEDDED_GLFW3) {
    ...
    if (key >= GLFW_KEY_KP_0 && key <= GLFW_KEY_KP_EQUAL)   do return key
...
ImGui_ImplGlfw_KeyCallback :: proc(window : ^GLFWwindow, keycode : i32, scancode : i32, action : i32, mods : i32)
{
    ...
    if (action != GLFW_PRESS && action != GLFW_RELEASE)   do return
...
ImGui_ImplGlfw_CursorPosCallback :: proc(window : ^GLFWwindow, x : f64, y : f64)
{
    ...
    bd.LastValidMousePos = ImVec2{(f32}x, cast(f32) y);
}
...
ImGui_ImplGlfw_MonitorCallback :: proc(GLFWmonitor*, i32)
{
    bd := ImGui_ImplGlfw_GetBackendData();
...
ImGui_ImplEmscripten_WheelCallback :: proc(i32, ev : ^EmscriptenWheelEvent, rawptr) -> EM_BOOL
{
    ...
    //IMGUI_DEBUG_LOG("[Emsc] mode %d dx: %.2f, dy: %.2f, dz: %.2f --> feed %.2f %.2f\n", cast(int) ev.deltaMode, ev.deltaX, ev.deltaY, ev.deltaZ, wheel_x, wheel_y);
    return EM_TRUE;
}
}
...
when GLFW_HAS_GETERROR && !defined(__EMSCRIPTEN__) { // Eat errors (see #5908)
    ...
    main_viewport.PlatformHandle = cast(rawptr) bd.Window;
when _WIN32 {
    main_viewport.PlatformHandleRaw = glfwGetWin32Window(bd.Window);
} else when defined(__APPLE__) {
    main_viewport.PlatformHandleRaw = cast(rawptr) glfwGetCocoaWindow(bd.Window);
} else {
    _ = main_viewport;
}
...
when EMSCRIPTEN_USE_EMBEDDED_GLFW3 {
        ...
        if (is_window_focused)
        {
            ...
            if (bd.MouseWindow == nullptr)
            {
                ...
                bd.LastValidMousePos = ImVec2{(f32}mouse_x, cast(f32) mouse_y);
...
when GLFW_HAS_MOUSE_PASSTHROUGH {
        window_no_input := (.NoInputs in viewport.Flags) != 0;
...
ImGui_ImplGlfw_UpdateMouseCursor :: proc()
{
    io := GetIO();
    bd := ImGui_ImplGlfw_GetBackendData();
    if ((.NoMouseCursorChange in io.ConfigFlags) || glfwGetInputMode(bd.Window, GLFW_CURSOR) == GLFW_CURSOR_DISABLED)   do return
...
when GLFW_HAS_GAMEPAD_API && !defined(EMSCRIPTEN_USE_EMBEDDED_GLFW3) {
    ...
    if (!glfwGetGamepadState(GLFW_JOYSTICK_1, &gamepad))   do return
    ...
    if (axes_count == 0 || buttons_count == 0)   do return
    ...
    MAP_ANALOG(ImGuiKey.GamepadL2,          GLFW_GAMEPAD_AXIS_LEFT_TRIGGER,     4,      -0.75,  +1.0);
    MAP_ANALOG(ImGuiKey.GamepadR2,          GLFW_GAMEPAD_AXIS_RIGHT_TRIGGER,    5,      -0.75,  +1.0);
    ...
    MAP_ANALOG(ImGuiKey.GamepadLStickLeft,  GLFW_GAMEPAD_AXIS_LEFT_X,           0,      -0.25,  -1.0);
    MAP_ANALOG(ImGuiKey.GamepadLStickRight, GLFW_GAMEPAD_AXIS_LEFT_X,           0,      +0.25,  +1.0);
    MAP_ANALOG(ImGuiKey.GamepadLStickUp,    GLFW_GAMEPAD_AXIS_LEFT_Y,           1,      -0.25,  -1.0);
    MAP_ANALOG(ImGuiKey.GamepadLStickDown,  GLFW_GAMEPAD_AXIS_LEFT_Y,           1,      +0.25,  +1.0);
    MAP_ANALOG(ImGuiKey.GamepadRStickLeft,  GLFW_GAMEPAD_AXIS_RIGHT_X,          2,      -0.25,  -1.0);
    MAP_ANALOG(ImGuiKey.GamepadRStickRight, GLFW_GAMEPAD_AXIS_RIGHT_X,          2,      +0.25,  +1.0);
    MAP_ANALOG(ImGuiKey.GamepadRStickUp,    GLFW_GAMEPAD_AXIS_RIGHT_Y,          3,      -0.25,  -1.0);
    MAP_ANALOG(ImGuiKey.GamepadRStickDown,  GLFW_GAMEPAD_AXIS_RIGHT_Y,          3,      +0.25,  +1.0);
    #undef MAP_BUTTON
    #undef MAP_ANALOG
}
...
ImGui_ImplGlfw_UpdateMonitors :: proc()
{
    ...
    for int n = 0; n < monitors_count; n += 1
    {
        ...
        monitor.MainPos = monitor.WorkPos = ImVec2{(f32}x, cast(f32) y);
        monitor.MainSize = monitor.WorkSize = ImVec2{(f32}vid_mode.width, cast(f32) vid_mode.height);
when GLFW_HAS_MONITOR_WORK_AREA {
        w, h : i32
        glfwGetMonitorWorkarea(glfw_monitors[n], &x, &y, &w, &h);
        if (w > 0 && h > 0) // Workaround a small GLFW issue reporting zero on monitor changes: https://github.com/glfw/glfw/pull/1761
        {
            monitor.WorkPos = ImVec2{(f32}x, cast(f32) y);
            monitor.WorkSize = ImVec2{(f32}w, cast(f32) h);
        }
}
when GLFW_HAS_PER_MONITOR_DPI {
        ...
        monitor.PlatformHandle = cast(rawptr) glfw_monitors[n]; // [...] GLFW doc states: "guaranteed to be valid only until the monitor configuration changes"
...
ImGui_ImplGlfw_NewFrame :: proc()
{
    ...
    if (w > 0 && h > 0)
        io.DisplayFramebufferScale = ImVec2{(f32}display_w / cast(f32) w, cast(f32) display_h / cast(f32) h);
...
ImGui_ImplGlfw_OnCanvasSizeChange :: proc(event_type : i32, event : ^EmscriptenUiEvent, user_data : rawptr) -> EM_BOOL
{
    ...
    glfwSetWindowSize(bd.Window, cast(i32) canvas_width, cast(i32) canvas_height);
    return true;
}
...
when GLFW_HAS_FOCUS_ON_SHOW {
    ...
    glfwWindowHint(GLFW_DECORATED, (.NoDecoration in viewport.Flags) ? false : true);
when GLFW_HAS_WINDOW_TOPMOST {
    glfwWindowHint(GLFW_FLOATING, (.TopMost in viewport.Flags) ? true : false);
}
    share_window := (bd.ClientApi == GlfwClientApi_OpenGL) ? bd.Window : nullptr;
    vd.Window = glfwCreateWindow(cast(i32) viewport.Size.x, cast(i32) viewport.Size.y, "No Title Yet", nullptr, share_window);
    vd.WindowOwned = true;
    viewport.PlatformHandle = cast(rawptr) vd.Window;
when _WIN32 {
    viewport.PlatformHandleRaw = glfwGetWin32Window(vd.Window);
} else when defined(__APPLE__) {
    viewport.PlatformHandleRaw = cast(rawptr) glfwGetCocoaWindow(vd.Window);
}
    glfwSetWindowPos(vd.Window, cast(i32) viewport.Pos.x, cast(i32) viewport.Pos.y);
...
when !GLFW_HAS_MOUSE_PASSTHROUGH && GLFW_HAS_WINDOW_HOVERED && defined(_WIN32) {
            ...
            for int i = 0; i < len(bd.KeyOwnerWindows); i += 1
...
ImGui_ImplGlfw_GetWindowPos :: proc(viewport : ^ImGuiViewport) -> ImVec2
{
    vd := (ImGui_ImplGlfw_ViewportData*)viewport.PlatformUserData;
    x := 0, y = 0;
...
ImGui_ImplGlfw_SetWindowPos :: proc(viewport : ^ImGuiViewport, pos : ImVec2)
{
    ...
    glfwSetWindowPos(vd.Window, cast(i32) pos.x, cast(i32) pos.y);
}

ImGui_ImplGlfw_GetWindowSize :: proc(viewport : ^ImGuiViewport) -> ImVec2
{
    vd := (ImGui_ImplGlfw_ViewportData*)viewport.PlatformUserData;
    w := 0, h = 0;
...
when __APPLE__ && !GLFW_HAS_OSX_WINDOW_POS_FIX {
    ...
    glfwSetWindowSize(vd.Window, cast(i32) size.x, cast(i32) size.y);
}
...
when _WIN32 {
GetMouseSourceFromMessageExtraInfo :: proc() -> ImGuiMouseSource
{
...
when !GLFW_HAS_MOUSE_PASSTHROUGH && GLFW_HAS_WINDOW_HOVERED {
    case WM_NCHITTEST:
    {
        ...
        if (viewport && (.NoInputs in viewport.Flags))   do return HTTRANSPARENT
        break;
    }
}
    }
