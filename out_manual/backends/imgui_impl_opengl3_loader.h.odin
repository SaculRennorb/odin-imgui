...
#define GL3W_ARRAY_SIZE(x)  (size_of(x) / size_of((x)[0]))
...
open_libgl :: proc(void) -> i32
{
    ...
    if (!libgl)   do return GL3W_ERROR_LIBRARY_OPEN
...
open_libs :: proc(void) -> i32
{
    ...
    if (is_library_loaded("libEGL.so.1", &libegl) ||
        ...
        if (libgl)   do return GL3W_OK
        else
            close_libgl();
    }

    if (is_library_loaded("libGL.so", &libgl))   do return GL3W_OK
    if (is_library_loaded("libGL.so.1", &libgl))   do return GL3W_OK
    if (is_library_loaded("libGL.so.3", &libgl))   do return GL3W_OK
    ...
    if (libgl && libegl)   do return GL3W_OK
    else
        close_libgl();
...
{
    if (!glGetIntegerv)   do return GL3W_ERROR_INIT
    glGetIntegerv(GL_MAJOR_VERSION, &version.major);
    glGetIntegerv(GL_MINOR_VERSION, &version.minor);
    if (version.major == 0 && version.minor == 0)
    {
        // Query GL_VERSION in desktop GL 2.x, the string will start with "<major>.<minor>"
        if (gl_version := cast(^u8) glGetString(GL_VERSION)) {
            sscanf(gl_version, "%d.%d", &version.major, &version.minor);
        }

    }
    if (version.major < 2)   do return GL3W_ERROR_OPENGL_VERSION
    return GL3W_OK;
}

void load_procs(GL3WGetProcAddressProc proc);

imgl3wInit :: proc(void) -> i32
{
    res := open_libgl();
    if (res)   do return res
    atexit(close_libgl);
...
{
    if (major < 2)   do return 0
    if (version.major == major)   do return version.minor >= minor
...
{
    i : int
    for i = 0; i < GL3W_ARRAY_SIZE(proc_names); i += 1
