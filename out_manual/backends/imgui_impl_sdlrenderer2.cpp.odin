...
ImGui_ImplSDLRenderer2_RenderDrawData :: proc(draw_data : ^ImDrawData, renderer : ^SDL_Renderer)
{
	...
	fb_width := (i32)(draw_data.DisplaySize.x * render_scale.x);
	fb_height := (i32)(draw_data.DisplaySize.y * render_scale.y);
    ...
    for int n = 0; n < draw_data.CmdListsCount; n += 1
    {
        ...
        for int cmd_i = 0; cmd_i < len(draw_list.CmdBuffer); cmd_i += 1
        {
            ...
            else
            {
                ...
                if (clip_max.x > cast(f32) fb_width) { clip_max.x = cast(f32) fb_width; }
                if (clip_max.y > cast(f32) fb_height) { clip_max.y = cast(f32) fb_height; }
                ...
                const f32* xy = (const f32*)(const rawptr)(cast(^u8) (vtx_buffer + pcmd.VtxOffset) + offset_of(ImDrawVert, pos));
                const f32* uv = (const f32*)(const rawptr)(cast(^u8) (vtx_buffer + pcmd.VtxOffset) + offset_of(ImDrawVert, uv));
when SDL_VERSION_ATLEAST(2,0,19) {
                const SDL_Color* color = (const SDL_Color*)(const rawptr)(cast(^u8) (vtx_buffer + pcmd.VtxOffset) + offset_of(ImDrawVert, col)); // SDL 2.0.19+
} else {
                const i32* color = (const i32*)(const rawptr)(cast(^u8) (vtx_buffer + pcmd.VtxOffset) + offset_of(ImDrawVert, col)); // SDL 2.0.17 and 2.0.18
}

                // Bind texture, Draw
				SDL_Texture* tex = (SDL_Texture*)pcmd.GetTexID();
                SDL_RenderGeometryRaw(renderer, tex,
                    xy, cast(int) size_of(ImDrawVert),
                    color, cast(int) size_of(ImDrawVert),
                    uv, cast(int) size_of(ImDrawVert),
                    len(draw_list.VtxBuffer) - pcmd.VtxOffset,
