...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, i32 w, i32 h, u32 min_image_count);
void ImGui_ImplVulkanH_CreateWindowCommandBuffers(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, u32 queue_family, const VkAllocationCallbacks* allocator);

// Vulkan prototypes for use with custom loaders
// (see description of IMGUI_IMPL_VULKAN_NO_PROTOTYPES in imgui_impl_vulkan.h
when defined(VK_NO_PROTOTYPES) && !defined(VOLK_H_) {
IMGUI_IMPL_VULKAN_USE_LOADER :: true
bool g_FunctionsLoaded = false;
} else {
bool g_FunctionsLoaded = true;
}
when IMGUI_IMPL_VULKAN_USE_LOADER {
...
ImGui_ImplVulkan_Texture :: struct
{
    ...
    ImGui_ImplVulkan_Texture() { memset((rawptr)this, 0, size_of(*this)); }
};

// For multi-viewport support:
// Helper structure we store in the void* RendererUserData field of each ImGuiViewport to easily retrieve our backend data.
ImGui_ImplVulkan_ViewportData :: struct
{
    Window : ImGui_ImplVulkanH_Window,                 // Used by secondary viewports only
    RenderBuffers : ImGui_ImplVulkan_WindowRenderBuffers,          // Used by all viewports
    WindowOwned : bool,
    SwapChainNeedRebuild : bool,   // Flag when viewport swapchain resized in the middle of processing a frame

    ImGui_ImplVulkan_ViewportData() { WindowOwned = SwapChainNeedRebuild = false; memset(&RenderBuffers, 0, size_of(RenderBuffers)); }
...
ImGui_ImplVulkan_Data :: struct
{
    ...
    Pipeline : VkPipeline,               // pipeline for main render pass (created by app)
    PipelineForViewports : VkPipeline,   // pipeline for secondary viewports (created by backend)
    ShaderModuleVert : VkShaderModule,
    ShaderModuleFrag : VkShaderModule,
    DescriptorPool : VkDescriptorPool,

    // Texture management
    FontTexture : ImGui_ImplVulkan_Texture,
    TexSampler : VkSampler,
    TexCommandPool : VkCommandPool,
    TexCommandBuffer : VkCommandBuffer,
...
ImGui_ImplVulkan_GetBackendData :: proc() -> ^ImGui_ImplVulkan_Data
{
    return GetCurrentContext() ? (ImGui_ImplVulkan_Data*)GetIO().BackendRendererUserData : nullptr;
}

ImGui_ImplVulkan_MemoryType :: proc(properties : VkMemoryPropertyFlags, type_bits : u32) -> u32
{
    ...
    for uint32_t i = 0; i < prop.memoryTypeCount; i += 1
        if ((prop.memoryTypes[i].propertyFlags & properties) == properties && type_bits & (1 << i))   do return i
    return 0xFFFFFFFF; // Unable to find memoryType
}

check_vk_result :: proc(err : VkResult)
{
    bd := ImGui_ImplVulkan_GetBackendData();
    if (!bd)   do return
    v := &bd.VulkanInitInfo;
    if (v.CheckVkResultFn)   do v.CheckVkResultFn(err)
}

// Same as IM_MEMALIGN(). 'alignment' must be a power of two.
inline VkDeviceSize AlignBufferSize(VkDeviceSize size, VkDeviceSize alignment)
{
    return (size + alignment - 1) & ~(alignment - 1);
}

CreateOrResizeBuffer :: proc(buffer : ^VkBuffer, buffer_memory : ^VkDeviceMemory, buffer_size : ^VkDeviceSize, new_size : VkDeviceSize, usage : VkBufferUsageFlagBits)
{
    bd := ImGui_ImplVulkan_GetBackendData();
    v := &bd.VulkanInitInfo;
    err : VkResult
    ...
    buffer_size_aligned := AlignBufferSize(IM_MAX(v.MinAllocationSize, new_size), bd.BufferMemoryAlignment);
    buffer_info := {};
    buffer_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    len(buffer_info) = buffer_size_aligned;
    ...
    alloc_info.allocationSize = len(req);
...
ImGui_ImplVulkan_SetupRenderState :: proc(draw_data : ^ImDrawData, pipeline : VkPipeline, command_buffer : VkCommandBuffer, rb : ^ImGui_ImplVulkan_FrameRenderBuffers, fb_width : i32, fb_height : i32)
{
    bd := ImGui_ImplVulkan_GetBackendData();
    ...
    if (draw_data.TotalVtxCount > 0)
    {
        ...
        vkCmdBindIndexBuffer(command_buffer, rb.IndexBuffer, 0, size_of(ImDrawIdx) == 2 ? VK_INDEX_TYPE_UINT16 : VK_INDEX_TYPE_UINT32);
    }
    ...
    // Our visible imgui space lies from draw_data.DisplayPps (top left) to draw_data.DisplayPos+data_data.DisplaySize (bottom right). DisplayPos is (0,0) for single viewport apps.
    {
        ...
        vkCmdPushConstants(command_buffer, bd.PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, size_of(f32) * 0, size_of(f32) * 2, scale);
        vkCmdPushConstants(command_buffer, bd.PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, size_of(f32) * 2, size_of(f32) * 2, translate);
    }
}

// Render function
ImGui_ImplVulkan_RenderDrawData :: proc(draw_data : ^ImDrawData, command_buffer : VkCommandBuffer, pipeline : VkPipeline)
{
    ...
    bd := ImGui_ImplVulkan_GetBackendData();
    v := &bd.VulkanInitInfo;
    if (pipeline == VK_NULL_HANDLE)   do pipeline = bd.Pipeline

    // Allocate array to store enough vertex/index buffers. Each unique viewport gets its own storage.
    viewport_renderer_data := (ImGui_ImplVulkan_ViewportData*)draw_data.OwnerViewport.RendererUserData;
    ...
    if (wrb.FrameRenderBuffers == nullptr)
    {
        ...
        wrb.FrameRenderBuffers = (ImGui_ImplVulkan_FrameRenderBuffers*)IM_ALLOC(size_of(ImGui_ImplVulkan_FrameRenderBuffers) * wrb.Count);
        memset((rawptr)wrb.FrameRenderBuffers, 0, size_of(ImGui_ImplVulkan_FrameRenderBuffers) * wrb.Count);
    }
    ...
    if (draw_data.TotalVtxCount > 0)
    {
        ...
        vertex_size := AlignBufferSize(draw_data.TotalVtxCount * size_of(ImDrawVert), bd.BufferMemoryAlignment);
        index_size := AlignBufferSize(draw_data.TotalIdxCount * size_of(ImDrawIdx), bd.BufferMemoryAlignment);
        ...
        vtx_dst := nullptr;
        idx_dst := nullptr;
        err := vkMapMemory(v.Device, rb.VertexBufferMemory, 0, vertex_size, 0, (rawptr*)&vtx_dst);
        check_vk_result(err);
        err = vkMapMemory(v.Device, rb.IndexBufferMemory, 0, index_size, 0, (rawptr*)&idx_dst);
        check_vk_result(err);
    ...
    render_state : ImGui_ImplVulkan_RenderState
    ...
    for int n = 0; n < draw_data.CmdListsCount; n += 1
    {
        ...
        for int cmd_i = 0; cmd_i < len(draw_list.CmdBuffer); cmd_i += 1
        {
            ...
            else
            {
                ...
                if (clip_max.x > fb_width) { clip_max.x = cast(f32) fb_width; }
                if (clip_max.y > fb_height) { clip_max.y = cast(f32) fb_height; }
    ...
    scissor := { { 0, 0 }, { cast(u32) fb_width, cast(u32) fb_height } };
    vkCmdSetScissor(command_buffer, 0, 1, &scissor);
}
...
ImGui_ImplVulkan_CreateFontsTexture :: proc() -> bool
{
    ...
    {
        map := nullptr;
        err = vkMapMemory(v.Device, upload_buffer_memory, 0, upload_size, 0, (rawptr*)(&map));
        check_vk_result(err);
...
ImGui_ImplVulkan_CreateShaderModules :: proc(device : VkDevice, allocator : ^VkAllocationCallbacks)
{
    ...
    if (bd.ShaderModuleVert == VK_NULL_HANDLE)
    {
        ...
        vert_info.codeSize = size_of(__glsl_shader_vert_spv);
        vert_info.pCode = (u32*)__glsl_shader_vert_spv;
    ...
    if (bd.ShaderModuleFrag == VK_NULL_HANDLE)
    {
        ...
        frag_info.codeSize = size_of(__glsl_shader_frag_spv);
        frag_info.pCode = (u32*)__glsl_shader_frag_spv;
        err := vkCreateShaderModule(device, &frag_info, allocator, &bd.ShaderModuleFrag);
        check_vk_result(err);
    }
}

ImGui_ImplVulkan_CreatePipeline :: proc(device : VkDevice, allocator : ^VkAllocationCallbacks, pipelineCache : VkPipelineCache, renderPass : VkRenderPass, MSAASamples : VkSampleCountFlagBits, pipeline : ^VkPipeline, subpass : u32)
{
    bd := ImGui_ImplVulkan_GetBackendData();
    ...
    binding_desc[0].stride = size_of(ImDrawVert);
    ...
    attribute_desc[0].offset = offset_of(ImDrawVert, pos);
    attribute_desc[1].location = 1;
    attribute_desc[1].binding = binding_desc[0].binding;
    attribute_desc[1].format = VK_FORMAT_R32G32_SFLOAT;
    attribute_desc[1].offset = offset_of(ImDrawVert, uv);
    attribute_desc[2].location = 2;
    attribute_desc[2].binding = binding_desc[0].binding;
    attribute_desc[2].format = VK_FORMAT_R8G8B8A8_UNORM;
    attribute_desc[2].offset = offset_of(ImDrawVert, col);
    ...
    dynamic_state.dynamicStateCount = cast(u32) len(dynamic_states);
...
when IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING {
    ...
    {
        assert(bd.VulkanInitInfo.PipelineRenderingCreateInfo.sType == VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR, "PipelineRenderingCreateInfo sType must be VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR");
        assert(bd.VulkanInitInfo.PipelineRenderingCreateInfo.pNext == nullptr, "PipelineRenderingCreateInfo pNext must be nullptr");
...
ImGui_ImplVulkan_CreateDeviceObjects :: proc() -> bool
{
    bd := ImGui_ImplVulkan_GetBackendData();
    v := &bd.VulkanInitInfo;
    err : VkResult
    ...
    if (!bd.PipelineLayout)
    {
        ...
        push_constants[0].offset = size_of(f32) * 0;
        push_constants[0].size = size_of(f32) * 4;
...
ImGui_ImplVulkan_DestroyDeviceObjects :: proc()
{
    bd := ImGui_ImplVulkan_GetBackendData();
    v := &bd.VulkanInitInfo;
...
ImGui_ImplVulkan_LoadFunctions :: proc(PFN_vkVoidFunction(*loader_func)(function_name : ^u8, user_data : rawptr), user_data : rawptr) -> bool
{
...
when !(IMGUI_IMPL_VULKAN_USE_LOADER) {
        ...
        assert(ImGuiImplVulkanFuncs_vkCmdBeginRenderingKHR != nullptr);
        assert(ImGuiImplVulkanFuncs_vkCmdEndRenderingKHR != nullptr);
} else {
        assert(false, "Can't use dynamic rendering when neither VK_VERSION_1_3 or VK_KHR_dynamic_rendering is defined.");
}
    }
...
ImGui_ImplVulkan_Shutdown :: proc()
{
    ...
    main_viewport := GetMainViewport();
    if (ImGui_ImplVulkan_ViewportData* vd = (ImGui_ImplVulkan_ViewportData*)main_viewport.RendererUserData)   do IM_DELETE(vd)
...
{
    bd := ImGui_ImplVulkan_GetBackendData();
    assert(bd != nullptr, "Context or backend not initialized! Did you call ImGui_ImplVulkan_Init()?");
...
ImGui_ImplVulkan_SetMinImageCount :: proc(min_image_count : u32)
{
    ...
    if (bd.VulkanInitInfo.MinImageCount == min_image_count)   do return
...
ImGui_ImplVulkan_AddTexture :: proc(sampler : VkSampler, image_view : VkImageView, image_layout : VkImageLayout) -> VkDescriptorSet
{
    bd := ImGui_ImplVulkan_GetBackendData();
    v := &bd.VulkanInitInfo;
    pool := bd.DescriptorPool ? bd.DescriptorPool : v.DescriptorPool;
...
ImGui_ImplVulkan_RemoveTexture :: proc(descriptor_set : VkDescriptorSet)
{
    bd := ImGui_ImplVulkan_GetBackendData();
    v := &bd.VulkanInitInfo;
    pool := bd.DescriptorPool ? bd.DescriptorPool : v.DescriptorPool;
...
ImGui_ImplVulkan_DestroyFrameRenderBuffers :: proc(device : VkDevice, buffers : ^ImGui_ImplVulkan_FrameRenderBuffers, allocator : ^VkAllocationCallbacks)
{
...
ImGui_ImplVulkan_DestroyWindowRenderBuffers :: proc(device : VkDevice, buffers : ^ImGui_ImplVulkan_WindowRenderBuffers, allocator : ^VkAllocationCallbacks)
{
    for u32 n = 0; n < buffers.Count; n += 1
...
ImGui_ImplVulkanH_SelectSurfaceFormat :: proc(physical_device : VkPhysicalDevice, surface : VkSurfaceKHR, request_formats : ^VkFormat, request_formats_count : i32, request_color_space : VkColorSpaceKHR) -> VkSurfaceFormatKHR
{
    assert(g_FunctionsLoaded, "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
    ...
    avail_format.resize(cast(i32) avail_count);
    ...
    else
    {
        ...
        for request_i := 0; request_i < request_formats_count; request_i += 1
            for u32 avail_i = 0; avail_i < avail_count; avail_i += 1
                if (avail_format[avail_i].format == request_formats[request_i] && avail_format[avail_i].colorSpace == request_color_space)   do return avail_format[avail_i]
...
ImGui_ImplVulkanH_SelectPresentMode :: proc(physical_device : VkPhysicalDevice, surface : VkSurfaceKHR, request_modes : ^VkPresentModeKHR, request_modes_count : i32) -> VkPresentModeKHR
{
    assert(g_FunctionsLoaded, "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
    ...
    avail_modes.resize(cast(i32) avail_count);
    vkGetPhysicalDeviceSurfacePresentModesKHR(physical_device, surface, &avail_count, avail_modes.Data);
    //for uint32_t avail_i = 0; avail_i < avail_count; avail_i += 1
    //    printf("[vulkan] avail_modes[%d] = %d\n", avail_i, avail_modes[avail_i]);

    for request_i := 0; request_i < request_modes_count; request_i += 1
        for u32 avail_i = 0; avail_i < avail_count; avail_i += 1
            if (request_modes[request_i] == avail_modes[avail_i])   do return request_modes[request_i]

    return VK_PRESENT_MODE_FIFO_KHR; // Always available
}

ImGui_ImplVulkanH_SelectPhysicalDevice :: proc(instance : VkInstance) -> VkPhysicalDevice
{
    gpu_count : u32
    ...
    for &device in gpus
    {
        ...
        if (properties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)   do return device
    }

    // Use first GPU (Integrated) is a Discrete one is not available.
    if (gpu_count > 0)   do return gpus[0]
    return VK_NULL_HANDLE;
}


ImGui_ImplVulkanH_SelectQueueFamilyIndex :: proc(physical_device : VkPhysicalDevice) -> u32
{
    count : u32
    vkGetPhysicalDeviceQueueFamilyProperties(physical_device, &count, nullptr);
    ImVector<VkQueueFamilyProperties> queues_properties;
    queues_properties.resize(cast(i32) count);
    vkGetPhysicalDeviceQueueFamilyProperties(physical_device, &count, queues_properties.Data);
    for u32 i = 0; i < count; i += 1
        if (queues_properties[i].queueFlags & VK_QUEUE_GRAPHICS_BIT)   do return i
    return (u32)-1;
}

ImGui_ImplVulkanH_CreateWindowCommandBuffers :: proc(physical_device : VkPhysicalDevice, device : VkDevice, wd : ^ImGui_ImplVulkanH_Window, queue_family : u32, allocator : ^VkAllocationCallbacks)
{
    ...
    err : VkResult
    for u32 i = 0; i < wd.ImageCount; i += 1
    {
        fd := &wd.Frames[i];
        {
            info := {};
    ...
    for u32 i = 0; i < wd.SemaphoreCount; i += 1
    {
        fsd := &wd.FrameSemaphores[i];
        {
            info := {};
...
ImGui_ImplVulkanH_GetMinImageCountFromPresentMode :: proc(present_mode : VkPresentModeKHR) -> i32
{
    if (present_mode == VK_PRESENT_MODE_MAILBOX_KHR)   do return 3
    if (present_mode == VK_PRESENT_MODE_FIFO_KHR || present_mode == VK_PRESENT_MODE_FIFO_RELAXED_KHR)   do return 2
    if (present_mode == VK_PRESENT_MODE_IMMEDIATE_KHR)   do return 1
    assert(false)
    return 1;
}

// Also destroy old swap chain and in-flight frames data, if any.
ImGui_ImplVulkanH_CreateWindowSwapChain :: proc(physical_device : VkPhysicalDevice, device : VkDevice, wd : ^ImGui_ImplVulkanH_Window, allocator : ^VkAllocationCallbacks, w : i32, h : i32, min_image_count : u32)
{
    err : VkResult
    ...
    // Create Swapchain
    {
        ...
        assert(wd.Frames == nullptr && wd.FrameSemaphores == nullptr);
        wd.SemaphoreCount = wd.ImageCount + 1;
        wd.Frames = (ImGui_ImplVulkanH_Frame*)IM_ALLOC(size_of(ImGui_ImplVulkanH_Frame) * wd.ImageCount);
        wd.FrameSemaphores = (ImGui_ImplVulkanH_FrameSemaphores*)IM_ALLOC(size_of(ImGui_ImplVulkanH_FrameSemaphores) * wd.SemaphoreCount);
        memset((rawptr)wd.Frames, 0, size_of(wd.Frames[0]) * wd.ImageCount);
        memset((rawptr)wd.FrameSemaphores, 0, size_of(wd.FrameSemaphores[0]) * wd.SemaphoreCount);
    ...
    // Create The Image Views
    {
        ...
        image_range := { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 };
...
ImGui_ImplVulkanH_CreateOrResizeWindow :: proc(instance : VkInstance, physical_device : VkPhysicalDevice, device : VkDevice, wd : ^ImGui_ImplVulkanH_Window, queue_family : u32, allocator : ^VkAllocationCallbacks, width : i32, height : i32, min_image_count : u32)
{
    assert(g_FunctionsLoaded, "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");
    (void)instance;
...
ImGui_ImplVulkanH_DestroyWindow :: proc(instance : VkInstance, device : VkDevice, wd : ^ImGui_ImplVulkanH_Window, allocator : ^VkAllocationCallbacks)
{
...
ImGui_ImplVulkanH_DestroyFrame :: proc(device : VkDevice, fd : ^ImGui_ImplVulkanH_Frame, allocator : ^VkAllocationCallbacks)
{
...
ImGui_ImplVulkanH_DestroyFrameSemaphores :: proc(device : VkDevice, fsd : ^ImGui_ImplVulkanH_FrameSemaphores, allocator : ^VkAllocationCallbacks)
{
...
ImGui_ImplVulkanH_DestroyAllViewportsRenderBuffers :: proc(device : VkDevice, allocator : ^VkAllocationCallbacks)
{
    platform_io := &GetPlatformIO();
    for int n = 0; n < len(platform_io.Viewports); n += 1
...
ImGui_ImplVulkan_CreateWindow :: proc(viewport : ^ImGuiViewport)
{
    bd := ImGui_ImplVulkan_GetBackendData();
    vd := IM_NEW(ImGui_ImplVulkan_ViewportData)();
    ...
    // Create surface
    platform_io := &GetPlatformIO();
    err := (VkResult)platform_io.Platform_CreateVkSurface(viewport, cast(u64) v.Instance, (const rawptr)v.Allocator, (u64*)&wd.Surface);
    check_vk_result(err);
    ...
    if (res != VK_TRUE)
    {
        assert(false) // Error: no WSI support on physical device
        return;
    }
...
when IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING {
    for u32 n = 0; n < v.PipelineRenderingCreateInfo.colorAttachmentCount; n += 1
        requestSurfaceImageFormats.append(v.PipelineRenderingCreateInfo.pColorAttachmentFormats[n]);
}
    ...
    for format in defaultFormats
        requestSurfaceImageFormats.append(format);

    requestSurfaceColorSpace := VK_COLORSPACE_SRGB_NONLINEAR_KHR;
    wd.SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(v.PhysicalDevice, wd.Surface, requestSurfaceImageFormats.Data, cast(int) requestSurfaceImageFormats.Size, requestSurfaceColorSpace);

    // Select Present Mode
    // FIXME-VULKAN: Even thought mailbox seems to get us maximum framerate with a single window, it halves framerate with a second window etc. (w/ Nvidia and SDK 1.82.1)
    VkPresentModeKHR present_modes[] = { VK_PRESENT_MODE_MAILBOX_KHR, VK_PRESENT_MODE_IMMEDIATE_KHR, VK_PRESENT_MODE_FIFO_KHR };
    wd.PresentMode = ImGui_ImplVulkanH_SelectPresentMode(v.PhysicalDevice, wd.Surface, &present_modes[0], len(present_modes));
    //printf("[vulkan] Secondary window selected PresentMode = %d\n", wd.PresentMode);

    // Create SwapChain, RenderPass, Framebuffer, etc.
    wd.ClearEnable = (.NoRendererClear in viewport.Flags) ? false : true;
    wd.UseDynamicRendering = v.UseDynamicRendering;
    ImGui_ImplVulkanH_CreateOrResizeWindow(v.Instance, v.PhysicalDevice, v.Device, wd, v.QueueFamily, v.Allocator, cast(i32) viewport.Size.x, cast(i32) viewport.Size.y, v.MinImageCount);
    vd.WindowOwned = true;
...
ImGui_ImplVulkan_DestroyWindow :: proc(viewport : ^ImGuiViewport)
{
...
ImGui_ImplVulkan_SetWindowSize :: proc(viewport : ^ImGuiViewport, size : ImVec2)
{
    bd := ImGui_ImplVulkan_GetBackendData();
    vd := (ImGui_ImplVulkan_ViewportData*)viewport.RendererUserData;
    if (vd == nullptr) // This is nullptr for the main viewport (which is left to the user/app to handle)   do return
    v := &bd.VulkanInitInfo;
    vd.Window.ClearEnable = (.NoRendererClear in viewport.Flags) ? false : true;
    ImGui_ImplVulkanH_CreateOrResizeWindow(v.Instance, v.PhysicalDevice, v.Device, &vd.Window, v.QueueFamily, v.Allocator, cast(i32) size.x, cast(i32) size.y, v.MinImageCount);
}

ImGui_ImplVulkan_RenderWindow :: proc(viewport : ^ImGuiViewport, rawptr)
{
    bd := ImGui_ImplVulkan_GetBackendData();
    vd := (ImGui_ImplVulkan_ViewportData*)viewport.RendererUserData;
    wd := &vd.Window;
    ...
    fsd := &wd.FrameSemaphores[wd.SemaphoreIndex];
    {
        {
        ...
        }
        {
            ...
            memcpy(&wd.ClearValue.color.float32[0], &clear_color, 4 * size_of(f32));
        }
when IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING {
...
}
        {
            ...
            info.clearValueCount = (.NoRendererClear in viewport.Flags) ? 0 : 1;
            info.pClearValues = (.NoRendererClear in viewport.Flags) ? nullptr : &wd.ClearValue;
...
ImGui_ImplVulkan_SwapBuffers :: proc(viewport : ^ImGuiViewport, rawptr)
{
    bd := ImGui_ImplVulkan_GetBackendData();
    vd := (ImGui_ImplVulkan_ViewportData*)viewport.RendererUserData;
    wd := &vd.Window;
    ...
    {
        vd.SwapChainNeedRebuild = true;
        if (err == VK_ERROR_OUT_OF_DATE_KHR)   do return
    }
    else
    {
        check_vk_result(err);
...
ImGui_ImplVulkan_InitMultiViewportSupport :: proc()
{
    platform_io := &GetPlatformIO();
    if (GetIO().ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
        assert(platform_io.Platform_CreateVkSurface != nullptr, "Platform needs to setup the CreateVkSurface handler.");
    }
...
ImGui_ImplVulkan_ShutdownMultiViewportSupport :: proc()
{
