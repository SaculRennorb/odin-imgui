...
{
    if (res)   do wgpuBindGroupRelease(res)
    res = nullptr;
}
SafeRelease :: proc(res : ^WGPUBuffer)
{
    if (res)   do wgpuBufferRelease(res)
    res = nullptr;
}
...
{
    if (res)   do wgpuSamplerRelease(res)
    res = nullptr;
}
...
{
    if (res)   do wgpuTextureRelease(res)
    res = nullptr;
}
...
ImGui_ImplWGPU_SetupRenderState :: proc(draw_data : ^ImDrawData, ctx : WGPURenderPassEncoder, fr : ^FrameResources)
{
    ...
    // Our visible imgui space lies from draw_data.DisplayPos (top left) to draw_data.DisplayPos+data_data.DisplaySize (bottom right).
    {
        ...
        wgpuQueueWriteBuffer(bd.defaultQueue, bd.renderResources.Uniforms, offset_of(Uniforms, MVP), mvp, size_of(Uniforms::MVP));
        gamma : f32
        ...
        wgpuQueueWriteBuffer(bd.defaultQueue, bd.renderResources.Uniforms, offset_of(Uniforms, Gamma), &gamma, size_of(Uniforms::Gamma));
    }
...
ImGui_ImplWGPU_RenderDrawData :: proc(draw_data : ^ImDrawData, pass_encoder : WGPURenderPassEncoder)
{
    ...
    if (fb_width <= 0 || fb_height <= 0 || draw_data.CmdListsCount == 0)   do return
    ...
    for int n = 0; n < draw_data.CmdListsCount; n += 1
    {
        ...
        for int cmd_i = 0; cmd_i < len(draw_list.CmdBuffer); cmd_i += 1
        {
            ...
            else
            {
                ...
                wgpuRenderPassEncoderSetScissorRect(pass_encoder, cast(u32) clip_min.x, cast(u32) clip_min.y, (u32)(clip_max.x - clip_min.x), (u32)(clip_max.y - clip_min.y));
...
when IMGUI_IMPL_WEBGPU_BACKEND_DAWN {
    ...
    // Upload texture data
    {
        ...
        size := { cast(u32) width, cast(u32) height, 1 };
...
{
    bd := ImGui_ImplWGPU_GetBackendData();
    if (!bd.wgpuDevice)   do return false
    if (bd.pipelineState)
    ...
    {
        { WGPUVertexFormat_Float32x2, cast(u64) offset_of(ImDrawVert, pos), 0 },
        { WGPUVertexFormat_Float32x2, cast(u64) offset_of(ImDrawVert, uv),  1 },
        { WGPUVertexFormat_Unorm8x4,  cast(u64) offset_of(ImDrawVert, col), 2 },
    };
...
ImGui_ImplWGPU_InvalidateDeviceObjects :: proc()
{
    ...
    for u32 i = 0; i < bd.numFramesInFlight; i += 1
