...
//  [X] Platform: Keyboard support. Since 1.87 we are using the io.AddKeyEvent() function. Pass ImGuiKey values to all key functions e.g. ImGui::IsKeyPressed(ImGuiKey.Space). [Legacy VK_* values are obsolete since 1.87 and not supported since 1.91.5]
...
//  2024-07-08: Inputs: Fixed ImGuiKey.Mod_Super being mapped to VK_APPS instead of VK_LWIN||VK_RWIN. (#7768)
//  2023-10-05: Inputs: Added support for extra ImGuiKey values: F13 to F24 function keys, app back/forward keys.
//  2023-09-25: Inputs: Synthesize key-down event on key-up for VK_SNAPSHOT / ImGuiKey.PrintScreen as Windows doesn't emit it (same behavior as GLFW/SDL).
...
//  2018-01-08: Inputs: Added mapping for ImGuiKey.Insert.
...
when !(IMGUI_IMPL_WIN32_DISABLE_GAMEPAD) {
...
void ImGui_ImplWin32_ShutdownMultiViewportSupport();
...
when !(IMGUI_IMPL_WIN32_DISABLE_GAMEPAD) {
    ...
    ImGui_ImplWin32_Data()      { memset((rawptr)this, 0, size_of(*this)); }
};
...
{
    return GetCurrentContext() ? (ImGui_ImplWin32_Data*)GetIO().BackendPlatformUserData : nullptr;
}
ImGui_ImplWin32_GetBackendData :: proc(io : ^ImGuiIO) -> ^ImGui_ImplWin32_Data
{
...
ImGui_ImplWin32_UpdateKeyboardCodePage :: proc(io : ^ImGuiIO)
{
    ...
    bd := ImGui_ImplWin32_GetBackendData(io);
    keyboard_layout := ::GetKeyboardLayout(0);
    keyboard_lcid := MAKELCID(HIWORD(keyboard_layout), SORT_DEFAULT);
    if (::GetLocaleInfoA(keyboard_lcid, (LOCALE_RETURN_NUMBER | LOCALE_IDEFAULTANSICODEPAGE), (LPSTR)&bd.KeyboardCodePage, size_of(bd.KeyboardCodePage)) == 0) {
...
ImGui_ImplWin32_InitEx :: proc(hwnd : rawptr, platform_has_own_dc : bool) -> bool
{
    ...
    perf_frequency, perf_counter : INT64
    if (!::QueryPerformanceFrequency((LARGE_INTEGER*)&perf_frequency))   do return false
    if (!::QueryPerformanceCounter((LARGE_INTEGER*)&perf_counter))   do return false

    // Setup backend capabilities flags
    bd := IM_NEW(ImGui_ImplWin32_Data)();
    ...
    main_viewport.PlatformHandle = main_viewport.PlatformHandleRaw = cast(rawptr) bd.hWnd;
...
when !(IMGUI_IMPL_WIN32_DISABLE_GAMEPAD) {
    ...
    for int n = 0; n < len(xinput_dll_names); n += 1
...
IMGUI_IMPL_API bool     ImGui_ImplWin32_Init(rawptr hwnd)
{
    return ImGui_ImplWin32_InitEx(hwnd, false);
}

IMGUI_IMPL_API bool     ImGui_ImplWin32_InitForOpenGL(rawptr hwnd)
{
...
{
    if (.NoMouseCursorChange in io.ConfigFlags)   do return false
...
ImGui_ImplWin32_ProcessKeyEventsWorkarounds :: proc(io : ^ImGuiIO)
{
    ...
    if (IsKeyDown(ImGuiKey.LeftShift) && !IsVkDown(VK_LSHIFT))
        ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey.LeftShift, false, VK_LSHIFT);
    if (IsKeyDown(ImGuiKey.RightShift) && !IsVkDown(VK_RSHIFT))
        ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey.RightShift, false, VK_RSHIFT);

    // Sometimes WM_KEYUP for Win key is not passed down to the app (e.g. for Win+V on some setups, according to GLFW).
    if (IsKeyDown(ImGuiKey.LeftSuper) && !IsVkDown(VK_LWIN))
        ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey.LeftSuper, false, VK_LWIN);
    if (IsKeyDown(ImGuiKey.RightSuper) && !IsVkDown(VK_RWIN))
        ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey.RightSuper, false, VK_RWIN);
}

ImGui_ImplWin32_UpdateKeyModifiers :: proc(io : ^ImGuiIO)
{
    io.AddKeyEvent(ImGuiKey.Mod_Ctrl, IsVkDown(VK_CONTROL));
    io.AddKeyEvent(ImGuiKey.Mod_Shift, IsVkDown(VK_SHIFT));
    io.AddKeyEvent(ImGuiKey.Mod_Alt, IsVkDown(VK_MENU));
    io.AddKeyEvent(ImGuiKey.Mod_Super, IsVkDown(VK_LWIN) || IsVkDown(VK_RWIN));
}
...
ImGui_ImplWin32_FindViewportByPlatformHandle :: proc(platform_io : ^ImGuiPlatformIO, hwnd : HWND) -> ^ImGuiViewport
{
    ...
    for viewport in platform_io.Viewports
        if (viewport.PlatformHandle == hwnd)   do return viewport
    return nullptr;
}
...
ImGui_ImplWin32_UpdateMouseData :: proc(io : ^ImGuiIO, platform_io : ^ImGuiPlatformIO)
{
    ...
    is_app_focused := (focused_window && (focused_window == bd.hWnd || ::IsChild(focused_window, bd.hWnd) || ImGui_ImplWin32_FindViewportByPlatformHandle(platform_io, focused_window)));
    if (is_app_focused)
    {
        ...
        {
            pos := { cast(i32) io.MousePos.x, cast(i32) io.MousePos.y };
            if ((.ViewportsEnable not_in io.ConfigFlags)) {
        ...
        if (!io.WantSetMousePos && bd.MouseTrackedArea == 0 && has_mouse_screen_pos)
        {
            ...
            mouse_pos := mouse_screen_pos;
            if (!(.ViewportsEnable in io.ConfigFlags)) {
                ::ScreenToClient(bd.hWnd, &mouse_pos);
            }

            io.AddMousePosEvent(cast(f32) mouse_pos.x, cast(f32) mouse_pos.y);
        }
    }
...
when !(IMGUI_IMPL_WIN32_DISABLE_GAMEPAD) {
    bd := ImGui_ImplWin32_GetBackendData(io);
    //if ((.NavEnableGamepad not_in io.ConfigFlags)) // FIXME: Technically feeding gamepad shouldn't depend on this now that they are regular inputs.
    //    return;
    ...
    if (!bd.HasGamepad || bd.XInputGetState == nullptr || bd.XInputGetState(0, &xinput_state) != ERROR_SUCCESS)   do return
    io.BackendFlags |= ImGuiBackendFlags_HasGamepad;
...
BOOL CALLBACK ImGui_ImplWin32_UpdateMonitors_EnumFunc(HMONITOR monitor, HDC, LPRECT, LPARAM)
{
    info := {};
    info.cbSize = size_of(MONITORINFO);
    if (!::GetMonitorInfo(monitor, &info))   do return TRUE
    imgui_monitor : ImGuiPlatformMonitor
    imgui_monitor.MainPos = ImVec2{(f32}info.rcMonitor.left, cast(f32) info.rcMonitor.top);
    imgui_monitor.MainSize = ImVec2{(f32}(info.rcMonitor.right - info.rcMonitor.left), (f32)(info.rcMonitor.bottom - info.rcMonitor.top));
    imgui_monitor.WorkPos = ImVec2{(f32}info.rcWork.left, cast(f32) info.rcWork.top);
    imgui_monitor.WorkSize = ImVec2{(f32}(info.rcWork.right - info.rcWork.left), (f32)(info.rcWork.bottom - info.rcWork.top));
    imgui_monitor.DpiScale = ImGui_ImplWin32_GetDpiScaleForMonitor(monitor);
    imgui_monitor.PlatformHandle = cast(rawptr) monitor;
    if (imgui_monitor.DpiScale <= 0.0)   do return TRUE // Some accessibility applications are declaring virtual monitors with a DPI of 0, see #7902.
    io := &GetPlatformIO();
    if (.PRIMARY in info.dwFlags)
        io.Monitors.push_front(imgui_monitor);
    else
        io.Monitors.append(imgui_monitor);
    return TRUE;
}
...
{
    bd := ImGui_ImplWin32_GetBackendData();
    GetPlatformIO().Monitors.resize(0);
...
{
    bd := ImGui_ImplWin32_GetBackendData();
    assert(bd != nullptr, "Context or backend not initialized? Did you call ImGui_ImplWin32_Init()?");
    io := GetIO();
    ...
    io.DeltaTime = (f32)(current_time - bd.Time) / bd.TicksPerSecond;
    ...
    mouse_cursor := io.MouseDrawCursor ? ImGuiMouseCursor_None : GetMouseCursor();
...
ImGui_ImplWin32_KeyEventToImGuiKey :: proc(wParam : WPARAM, lParam : LPARAM) -> ImGuiKey
{
    ...
    if ((wParam == VK_RETURN) && (HIWORD(lParam) & KF_EXTENDED))   do return ImGuiKey.KeypadEnter
    ...
    switch (wParam)
    {
        ...
        case VK_F13: return ImGuiKey.F13;
        case VK_F14: return ImGuiKey.F14;
        case VK_F15: return ImGuiKey.F15;
        case VK_F16: return ImGuiKey.F16;
        case VK_F17: return ImGuiKey.F17;
        case VK_F18: return ImGuiKey.F18;
        case VK_F19: return ImGuiKey.F19;
        case VK_F20: return ImGuiKey.F20;
        case VK_F21: return ImGuiKey.F21;
        case VK_F22: return ImGuiKey.F22;
        case VK_F23: return ImGuiKey.F23;
        case VK_F24: return ImGuiKey.F24;
        case VK_BROWSER_BACK: return ImGuiKey.AppBack;
        case VK_BROWSER_FORWARD: return ImGuiKey.AppForward;
...
ImGui_ImplWin32_GetMouseSourceFromMessageExtraInfo :: proc() -> ImGuiMouseSource
{
    ...
    if ((extra_info & 0xFFFFFF80) == 0xFF515700)   do return ImGuiMouseSource_Pen
    if ((extra_info & 0xFFFFFF80) == 0xFF515780)   do return ImGuiMouseSource_TouchScreen
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (GetCurrentContext() == nullptr)   do return 0
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerEx(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, ImGuiIO& io)
{
    ...
    case WM_NCMOUSEMOVE:
    {
        ...
        mouse_source := ImGui_ImplWin32_GetMouseSourceFromMessageExtraInfo();
        area := (msg == WM_MOUSEMOVE) ? 1 : 2;
        bd.MouseHwnd = hwnd;
        if (bd.MouseTrackedArea != area)
        {
            tme_cancel := { size_of(tme_cancel), TME_CANCEL, hwnd, 0 };
            tme_track := { size_of(tme_track), (DWORD)((area == 2) ? (TME_LEAVE | TME_NONCLIENT) : TME_LEAVE), hwnd, 0 };
        ...
        mouse_pos := { (LONG)GET_X_LPARAM(lParam), (LONG)GET_Y_LPARAM(lParam) };
        want_absolute_pos := (.ViewportsEnable in io.ConfigFlags) != 0;
    ...
    {
        area := (msg == WM_MOUSELEAVE) ? 1 : 2;
        if (bd.MouseTrackedArea == area)
        {
            if (bd.MouseHwnd == hwnd)   do bd.MouseHwnd = nullptr
    ...
    case WM_XBUTTONUP:
    {
        ...
        bd.MouseButtonsDown &= ~(1 << button);
        if (bd.MouseButtonsDown == 0 && ::GetCapture() == hwnd)   do ::ReleaseCapture()
        io.AddMouseSourceEvent(mouse_source);
        io.AddMouseButtonEvent(button, false);
        return 0;
    }
    case WM_MOUSEWHEEL:
        io.AddMouseWheelEvent(0.0, cast(f32) GET_WHEEL_DELTA_WPARAM(wParam) / cast(f32) WHEEL_DELTA);
        return 0;
    case WM_MOUSEHWHEEL:
        io.AddMouseWheelEvent(-cast(f32) GET_WHEEL_DELTA_WPARAM(wParam) / cast(f32) WHEEL_DELTA, 0.0);
        return 0;
    case WM_KEYDOWN:
    case WM_KEYUP:
    case WM_SYSKEYDOWN:
    case WM_SYSKEYUP:
    {
        ...
        if (wParam < 256)
        {
            ...
            key := ImGui_ImplWin32_KeyEventToImGuiKey(wParam, lParam);
            vk := cast(i32) wParam;
            scancode := cast(i32) LOBYTE(HIWORD(lParam));

            // Special behavior for VK_SNAPSHOT / ImGuiKey.PrintScreen as Windows doesn't emit the key down event.
            ...
            if (vk == VK_SHIFT)
            {
                ...
                if (IsVkDown(VK_LSHIFT) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey.LeftShift, is_key_down, VK_LSHIFT, scancode); }
                if (IsVkDown(VK_RSHIFT) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey.RightShift, is_key_down, VK_RSHIFT, scancode); }
            }
            else if (vk == VK_CONTROL)
            {
                if (IsVkDown(VK_LCONTROL) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey.LeftCtrl, is_key_down, VK_LCONTROL, scancode); }
                if (IsVkDown(VK_RCONTROL) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey.RightCtrl, is_key_down, VK_RCONTROL, scancode); }
            }
            else if (vk == VK_MENU)
            {
                if (IsVkDown(VK_LMENU) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey.LeftAlt, is_key_down, VK_LMENU, scancode); }
                if (IsVkDown(VK_RMENU) == is_key_down) { ImGui_ImplWin32_AddKeyEvent(io, ImGuiKey.RightAlt, is_key_down, VK_RMENU, scancode); }
            }
        }
        return 0;
    }
    ...
    case WM_CHAR:
        if (::IsWindowUnicode(hwnd))
        {
            ...
            if (wParam > 0 && wParam < 0x10000)
                io.AddInputCharacterUTF16(cast(u16) wParam);
        }
        else
        {
            wch := 0;
            ::MultiByteToWideChar(bd.KeyboardCodePage, MB_PRECOMPOSED, (u8*)&wParam, 1, &wch, 1);
            io.AddInputCharacter(wch);
        }
        return 0;
    case WM_SETCURSOR:
        // This is required to restore cursor when transitioning from e.g resize borders to client area.
        if (LOWORD(lParam) == HTCLIENT && ImGui_ImplWin32_UpdateMouseCursor(io, bd.LastMouseCursor))   do return 1
        return 0;
...
_IsWindowsVersionOrGreater :: proc(major : WORD, minor : WORD, WORD) -> BOOL
{
    ...
    if (RtlVerifyVersionInfoFn == nullptr)   do return FALSE
...
when !(DPI_ENUMS_DECLARED) {
PROCESS_DPI_AWARENESS :: enum { PROCESS_DPI_UNAWARE = 0, PROCESS_SYSTEM_DPI_AWARE = 1, PROCESS_PER_MONITOR_DPI_AWARE = 2 }
MONITOR_DPI_TYPE :: enum { MDT_EFFECTIVE_DPI = 0, MDT_ANGULAR_DPI = 1, MDT_RAW_DPI = 2, MDT_DEFAULT = MDT_EFFECTIVE_DPI }
}
when !(_DPI_AWARENESS_CONTEXTS_) {
DECLARE_HANDLE(DPI_AWARENESS_CONTEXT);
#define DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE    (DPI_AWARENESS_CONTEXT)-3
}
when !(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2) {
...
{
    monitor := ::MonitorFromWindow((HWND)hwnd, MONITOR_DEFAULTTONEAREST);
...
{
    if (!_IsWindowsVistaOrGreater())   do return

    composition : BOOL
    if (FAILED(::DwmIsCompositionEnabled(&composition)) || !composition)   do return
...
ImGui_ImplWin32_ViewportData :: struct
{
    ...
    ~ImGui_ImplWin32_ViewportData() { assert(Hwnd == nullptr); }
};

ImGui_ImplWin32_GetWin32StyleFromViewportFlags :: proc(flags : ImGuiViewportFlags, out_style : ^DWORD, out_ex_style : ^DWORD)
{
    if (.NoDecoration in flags)   do out_style^ = WS_POPUP
    else
        out_style^ = WS_OVERLAPPEDWINDOW;

    if (.NoTaskBarIcon in flags) {
...
ImGui_ImplWin32_CreateWindow :: proc(viewport : ^ImGuiViewport)
{
    ...
    // Create window
    rect := { (LONG)viewport.Pos.x, (LONG)viewport.Pos.y, (LONG)(viewport.Pos.x + viewport.Size.x), (LONG)(viewport.Pos.y + viewport.Size.y) };
...
ImGui_ImplWin32_DestroyWindow :: proc(viewport : ^ImGuiViewport)
{
    ...
    if (ImGui_ImplWin32_ViewportData* vd = (ImGui_ImplWin32_ViewportData*)viewport.PlatformUserData)
    {
        ...
        if (vd.Hwnd && vd.HwndOwned)   do ::DestroyWindow(vd.Hwnd)
        vd.Hwnd = nullptr;
        IM_DELETE(vd);
    }
...
ImGui_ImplWin32_UpdateWindow :: proc(viewport : ^ImGuiViewport)
{
    ...
    if (vd.DwStyle != new_style || vd.DwExStyle != new_ex_style)
    {
        ...
        insert_after := top_most_changed ? ((.TopMost in viewport.Flags) ? HWND_TOPMOST : HWND_NOTOPMOST) : 0;
        swp_flag := top_most_changed ? 0 : SWP_NOZORDER;
...
ImGui_ImplWin32_GetWindowPos :: proc(viewport : ^ImGuiViewport) -> ImVec2
{
    ...
    return ImVec2{(f32}pos.x, cast(f32) pos.y);
}
...
ImGui_ImplWin32_SetWindowPos :: proc(viewport : ^ImGuiViewport, pos : ImVec2)
{
    ...
    if (.OwnedByApp in viewport.Flags)
...
ImGui_ImplWin32_GetWindowSize :: proc(viewport : ^ImGuiViewport) -> ImVec2
{
    ...
    return ImVec2{f32(rect.right - rect.left}, f32(rect.bottom - rect.top));
}
...
ImGui_ImplWin32_SetWindowSize :: proc(viewport : ^ImGuiViewport, size : ImVec2)
{
    vd := (ImGui_ImplWin32_ViewportData*)viewport.PlatformUserData;
    assert(vd.Hwnd != 0);
    rect := { 0, 0, (LONG)size.x, (LONG)size.y };
    if (.OwnedByApp in viewport.Flags)
        ImGui_ImplWin32_UpdateWin32StyleFromWindow(viewport); // Not our window, poll style before using
    ::AdjustWindowRectEx(&rect, vd.DwStyle, FALSE, vd.DwExStyle); // Client to Screen
...
ImGui_ImplWin32_SetWindowTitle :: proc(viewport : ^ImGuiViewport, title : ^u8)
{
    ...
    vd := (ImGui_ImplWin32_ViewportData*)viewport.PlatformUserData;
    assert(vd.Hwnd != 0);
    n := ::MultiByteToWideChar(CP_UTF8, 0, title, -1, nullptr, 0);
...
ImGui_ImplWin32_SetWindowAlpha :: proc(viewport : ^ImGuiViewport, alpha : f32)
{
    ...
    if (alpha < 1.0)
    {
        ex_style := ::GetWindowLongW(vd.Hwnd, GWL_EXSTYLE) | WS_EX_LAYERED;
...
LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    io := &GetIOEx(ctx);
    platform_io := &GetPlatformIOEx(ctx);
    result := 0;
    if (ImGui_ImplWin32_WndProcHandlerEx(hWnd, msg, wParam, lParam, io))   do result = true
    else if (ImGuiViewport* viewport = ImGui_ImplWin32_FindViewportByPlatformHandle(platform_io, hWnd))
    {
        ...
        case WM_MOUSEACTIVATE:
            if (.NoFocusOnClick in viewport.Flags)   do result = MA_NOACTIVATE
            break;
        case WM_NCHITTEST:
            ...
            if (.NoInputs in viewport.Flags)   do result = HTTRANSPARENT
            break;
        }
    }
    if (result == 0) {
